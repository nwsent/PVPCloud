...
object DataHelper {
    ...
    fun formatData(time: Long) = dateFormatSimple.format(Date(time)).toString()

    fun getTime(time: Long) = this.timeFormat.format(Date(time))
    ...
    fun parseLong(milliseconds: Long, abbreviate: Boolean): String? {
        ...
        for (s in units) {
            if (!abbreviate) {
                ...
                word = when (c) {
                    'w' -> "tygodni" + (if (count == "1") "" else "")
                    'd' -> "dni" + (if (count == "1") "" else "")
                    'h' -> "godzin" + (if (count == "1") "" else "")
                    'm' -> "minut" + (if (count == "1") "" else "")
                    else -> "sekund" + (if (count == "1") "" else "")
                }
