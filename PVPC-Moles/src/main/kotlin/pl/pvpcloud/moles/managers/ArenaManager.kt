...
import pl.pvpcloud.moles.arena.ArenaState
import java.io.File
...
class ArenaManager(private val plugin: MolesPlugin) {
    ...
    fun getArena(id: Int): Arena {
        return this.arenas[id]
                ?: throw NullPointerException("arena $id no exist")
    }
    ...
    fun load(arena: Arena) {
        arena.arenaState = ArenaState.RESTARTING
        ...
        arena.arenaState = ArenaState.WAITING
    }

    fun unload(arena: Arena) {
        arena.arenaState = ArenaState.NONE
